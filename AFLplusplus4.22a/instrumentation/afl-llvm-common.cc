...
void initInstrumentList() {
  ...
  if (allowlist) {
    ...
    while (getline(fileStream, line)) {
      ...
      if (line.find("src:") != std::string::npos && line.find("fun:") != std::string::npos) {
        std::string file = line.substr(4, line.find("fun:") - 4);
        std::string function = line.substr(line.find("fun:") + 4);

        if (file.empty() || function.empty()) {
          FATAL("invalid line in AFL_LLVM_ALLOWLIST: %s", original_line.c_str());
        }

        allowListFiles.push_back(file);
        allowListFunctions.push_back(function);

        if (debug) {
          DEBUGF("Added allowlist entry: file %s, function %s\n", file.c_str(), function.c_str());
        }
      } else {
        int is_file = -1;
        ...
        if (line.length() > 0) {
          ...
          if (debug) {
            DEBUGF("Added allowlist entry: %s\n", line.c_str());
          }
        } else {
          FATAL("invalid line in AFL_LLVM_ALLOWLIST: %s", original_line.c_str());
        }
    ...
    if (debug)
      DEBUGF("loaded allowlist with %zu entries\n", allowListFiles.size() + allowListFunctions.size());
  }
...
bool isInInstrumentList(llvm::Function *F, std::string Filename) {
  ...
  // if we do not have an instrument file list return true
  if (!allowListFiles.empty() || !allowListFunctions.empty()) {
    return_default = false;

    std::string instFunction = F->getName().str();
    std::string sourceFile = getSourceName(F);
    if (sourceFile.empty()) sourceFile = Filename;

    bool inAllowListFiles = false;
    bool inAllowListFunctions = false;

    // Check for combined file and function match if both lists are set
    if (!allowListFiles.empty() && !allowListFunctions.empty()) {
      for (auto itFile = allowListFiles.begin(), itFunc = allowListFunctions.begin();
           itFile != allowListFiles.end() && itFunc != allowListFunctions.end(); ++itFile, ++itFunc) {
        if ((sourceFile.length() >= itFile->length() &&
            sourceFile.compare(sourceFile.length() - itFile->length(), itFile->length(), *itFile) == 0) &&
            (fnmatch(("*" + *itFunc).c_str(), instFunction.c_str(), 0) == 0)) {
          if (debug) {
            DEBUGF("Function %s in file %s is in the combined allowlist, instrumenting ... \n", instFunction.c_str(), sourceFile.c_str());
          }
          return true;
        }
      }
    } else {
      // Check for individual file match
      for (const auto &allowFile : allowListFiles) {
        if (sourceFile.length() >= allowFile.length() &&
            sourceFile.compare(sourceFile.length() - allowFile.length(), allowFile.length(), allowFile) == 0) {
          inAllowListFiles = true;
          break;
        }
      }

      // Check for individual function match
      for (const auto &allowFunc : allowListFunctions) {
        if (fnmatch(("*" + allowFunc).c_str(), instFunction.c_str(), 0) == 0) {
          inAllowListFunctions = true;
          break;
        }
      }

      // Instrument if either file or function matches individually
      if (inAllowListFiles || inAllowListFunctions) {
        if (debug) {
          DEBUGF("Function %s or file %s is in the allowlist, instrumenting ... \n", instFunction.c_str(), sourceFile.c_str());
        }
        return true;
      }
    }
    ...
    // if (debug) {
    //   DEBUGF("No allowlist provided, instrumenting all functions by default.\n");
  }
