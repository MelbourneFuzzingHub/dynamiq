  ...
  while ((opt = getopt(argc, argv,
    ...
    switch (opt) {
      ...
      case 'F':                                         /* foreign sync dir */
        // Initialize processed_hashes if not already initialized
        if (afl->processed_hashes == NULL) {
            afl->processed_hashes = hash_set_new();  // Initialize the hash set
            if (!afl->processed_hashes) {
                FATAL("Failed to initialize processed_hashes");
            }
        }
