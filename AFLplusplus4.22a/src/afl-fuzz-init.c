...
void read_foreign_testcases(afl_state_t *afl, int first) {
  ...
  for (iter = 0; iter < afl->foreign_sync_cnt; iter++) {
    ...
    if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0]) {
      ...
      /* Scan the foreign sync directory */
      ...
      if (nl_cnt == 0) {
        if (first) {
          WARNF("directory %s is currently empty", afl->foreign_syncs[iter].dir);
        }
        free(nl);
        continue;
      }
      ...
      for (i = 0; i < (u32)nl_cnt; ++i) {
        ...
        /* Copy d_name BEFORE freeing nl[i] to avoid UAF */
        char dname_buf[NAME_MAX + 1];
        dname_buf[0] = 0;
        if (nl[i] && nl[i]->d_name) {
          strncpy(dname_buf, nl[i]->d_name, NAME_MAX);
          dname_buf[NAME_MAX] = 0;
        }
        free(nl[i]);  /* safe now, we kept dname in dname_buf */

        /* Skip "." and ".." */
        if (dname_buf[0] == '.' &&
            (dname_buf[1] == '\0' ||
             (dname_buf[1] == '.' && dname_buf[2] == '\0'))) {
          continue;
        }

        u8 *fn2 = alloc_printf("%s/%s", afl->foreign_syncs[iter].dir, dname_buf);
        if (!fn2) continue;

        struct stat st;
        if (unlikely(lstat((const char *)fn2, &st))) {
          if (first) PFATAL("Unable to access '%s'", fn2);
          ck_free(fn2);
          continue;
        }

        /* Ignore non-regular files, empty files, README and .metadata */
        if (!S_ISREG(st.st_mode) || !st.st_size ||
            !strcmp(dname_buf, "README.txt") || strstr(dname_buf, ".metadata")) {
          ck_free(fn2);
          continue;
        }

        if ((u64)st.st_size > (u64)MAX_FILE) {
          if (first) {
        ...
        /* === Dedupe key: name-truncated (if ends with -digits) OR SHA1(file) === */
        u8 *test_case_hash = get_test_case_hash(dname_buf);
        if (!test_case_hash) {
          ck_free(fn2);
          continue;
        }

        if (hash_set_contains(afl->processed_hashes, test_case_hash)) {
          ck_free(fn2);
          ck_free(test_case_hash);
          continue;  /* already processed */
        }

        /* Open and process the test case file */
        int fd = open((const char *)fn2, O_RDONLY | O_CLOEXEC);
        if (fd < 0) {
          ck_free(fn2);
          ck_free(test_case_hash);
          continue;
        ...
        u32 len = write_to_testcase(afl, (void **)&mem, (u32)st.st_size, 1);
        ...
        /* Mark this dedupe key as processed */
        hash_set_insert(afl->processed_hashes, test_case_hash);

        ck_free(fn2);
        ck_free(test_case_hash);
      }

      free(nl);  /* free the list allocated by scandir */
    }
  }
