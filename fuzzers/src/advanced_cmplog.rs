...
use libafl::prelude::{AFLppRedQueen, AflMapFeedback, CrashFeedback, StdMOptMutator, StdWeightedScheduler};
...
use libafl::corpus::{Corpus, CorpusMinimizer, InMemoryOnDiskCorpus, OnDiskCorpus, StdCorpusMinimizer};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Combine cmplog_dir with compiled binary name
    // let cmplog_binary_name = format!("{}/{}", cmplog_dir, args.binary_name);
    ...
    std::env::set_var("AFL_SHM_ID", shm_id);

    let edges_observer = unsafe {
        HitcountsMapObserver::new(StdMapObserver::new("shared_mem", shmem_buf)).track_indices()
    };

    // Clone the observer for later use in minimizer
    let edges_observer_clone = edges_observer.clone();
    ...
    //
    // Component: Corpus sync_edges_observer_clone
    //
    let minimizer = StdCorpusMinimizer::new(&edges_observer_clone);
